...
contract("AftermarketNonFungible", (accounts) => {
  ...
  it("should buy 2 tickets for acc0", async () => {
    ...
    var bigNumber1 = await event.tickets(ticketTypeId, eventGuests[0]);
  ...
  it("should post a non fungible ticket for sale", async () => {
    await event.makeSellOrderNonFungibles([ids[1]], [50], {
      from: eventGuests[0],
    });
    ...
    for(id of ids){
      var nfSellOrder = await event.nfTickets(id);
      if (nfSellOrder["userAddress"] != "0x00000000000000000000000000000000000000000"){
        console.log(nfSellOrder["userAddress"] + ": " + nfSellOrder["percentage"])
      }
    }
  });
