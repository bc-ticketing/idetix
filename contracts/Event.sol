...
contract Event {
    ...
    mapping(address => uint256) private affiliates;
     ...
     *
     */
    function setAffiliateComission(uint8 _commissionRate)
        public
        onlyEventOwner()
    {
    ...
    function addAffiliates(address[] memory _addresses)
        public
        onlyEventOwner()
    {
    ...
    function removeAffililates(address[] memory _addresses)
        public
        onlyEventOwner()
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    function claimERC20Comission() public onlyERC20Payments() {
        ERC20(erc20Address).transferFrom(
            address(this),
            msg.sender,
            getAffiliateCommision(msg.sender)
        );
    }

    function addAffiliateCommission(
        address[] memory _affiliates,
        uint256 _amount
    ) public {
        uint256 commission = ((_amount * affiliateCommission) / 100) /
            _affiliates.length;
    ...
    function getAffiliateCommision(address _affiliateAddress)
        public
        view
        returns (uint256)
    {
    ...
    modifier onlyERC20Payments() {
        require(
            ...
            "The event does not accept ETH payments."
        );
        _;
    }
    ...
    modifier onlyETHPayments() {
        require(
            erc20Address == address(0),
            "The event only accepts ETH payments."
        );
        _;
    }
    ...
    modifier onlyAffiliate() {
        require(
            ...
            "The address is not an approved affiliate."
        );
        _;
    }
}
